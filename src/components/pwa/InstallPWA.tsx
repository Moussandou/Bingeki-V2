...
const InstallButton = ({ variant, onInstall, className, style, t }: InstallButtonProps) => {
    if (variant === 'full') {
        return (
            <Button onClick={onInstall} variant="primary" icon={<Download size={18} />} className={className} style={style}>
                {t('pwa.install_app')}
            </Button>
        );
    }
