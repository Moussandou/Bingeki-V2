...
export const Podium: React.FC<PodiumProps> = ({ users, category }) => {
    ...
    const PodiumStep = ({ user, rank, delay }: { user: UserProfile | null, rank: 1 | 2 | 3, delay: number }) => {
    ...
    return (
        <div className={styles.container}>
            {/* Order: 2 - 1 - 3 */}
            <PodiumStep user={second} rank={2} delay={0.2} />
            <PodiumStep user={first} rank={1} delay={0.1} />
            <PodiumStep user={third} rank={3} delay={0.3} />
        </div>
    );
};
